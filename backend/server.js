...
function readTankConfig() {
  try {
    return JSON.parse(readFileSync(TANK_CONFIG_PATH, "utf8"));
  } catch {
    return {
      shape: "cylinder",
      maxDistanceCm: 200,
...
mqttClient.on("message", async (topic, message) => {
  // ── Hydrant Ultrasonik (ESP32 — plain text, bukan JSON) ──────────
  if (topic === "hydrant/jarak/air") {
    const raw = message.toString().trim();
    const distanceCm = parseFloat(raw);
    ...
    const waterLevelPct = Math.max(
      0,
      Math.min(100, ((maxDist - distanceCm) / maxDist) * 100),
    );

    // Update cache global
    latestSensorData.water_distance = distanceCm;
    latestSensorData.water_level = +waterLevelPct.toFixed(1);

    // Emit ke semua frontend client
    io.emit("sensor:update", {
      node_id: "hydrant_ultrasonic",
      water_distance: distanceCm,
      water_level: +waterLevelPct.toFixed(1),
      timestamp: new Date(),
    });

    console.log(
      `📡 Hydrant Ultrasonik: jarak=${distanceCm} cm, level=${waterLevelPct.toFixed(1)}%`,
    );
    return;
  }
  ...
  if (nodeId === undefined) {
    if (
      topic === "projek_orange_pi/sensor/node1" ||
      topic === "projek_orange_pi/sensor/master" ||
      topic === "projek_orange_pi/sensor/env"
    ) {
      nodeId = 1;
  ...
  let final_valve =
    gas_valve_status ?? water_valve_status ?? valve_status ?? null;
  if (final_valve === "TERBUKA" || final_valve === "OPEN") final_valve = "OPEN";
  if (final_valve === "TERTUTUP" || final_valve === "CLOSED")
    final_valve = "CLOSED";
  ...
  const pixelArray = Array.isArray(thermal_pixels)
    ? thermal_pixels.map(Number).filter((n) => !isNaN(n))
    : [];
  const maxFromPixels =
    pixelArray.length === 64 ? Math.max(...pixelArray) : null;
  ...
  const effectiveFlameStatus =
    binaryStatus(final_uv ?? 0) ?? normalizeStatus(flame_status) ?? "NORMAL";
  ...
  try {
    ...
    const [states] = await pool.execute(
      "SELECT device, status FROM actuator_state",
    );
    const currentStates = Object.fromEntries(
      states.map((s) => [s.device, s.status]),
    );
    ...
    updatedActuators = Object.fromEntries(
      updatedStates.map((s) => [s.device, s.status]),
    );
  ...
  Object.keys(emitPayload).forEach(
    (k) => emitPayload[k] === undefined && delete emitPayload[k],
  );
...
cron.schedule(
  ...
  async () => {
    try {
      ...
      console.log(
        `🕛 [CRON] water_usage snapshot: ${today} → ${volume} m³ (level: ${waterLevelCm?.toFixed(1)} cm)`,
      );
    } catch (err) {
...
server.listen(PORT, () => {
  ...
  console.log(
    `   POST  /api/sensor/data       — Receive sensor data from Orange Pi`,
  );
